...
.cyberarxiv_init_schema <- function(con) {
  ...
  # FIX: Removed unused umap_x, umap_y columns. Added ml_tag and ml_confidence
  # for MLflow classification integration.

  # Add ml_tag and ml_confidence columns if they don't exist (migration for existing DBs)
  existing_cols <- DBI::dbGetQuery(con, "SELECT column_name FROM information_schema.columns WHERE table_name = 'papers'")$column_name
