...
#'
#' Renders the Quarto dashboard. If source = NULL, uses embedded template.
#'
...
render_dashboard <- function(source = NULL,
  ...
  staging_root <- file.path(
    tempdir(),
    sprintf("cyberarxiv-dashboard-%s-%s", as.integer(Sys.time()), paste0(sample(letters, 4, replace = TRUE), collapse = ""))
  )
  ...
  if (dir.exists(rendered_assets)) {
    ...
    if (!file.copy(rendered_assets, output_dir, recursive = TRUE, overwrite = TRUE)) {
      warning("Failed to copy some dashboard assets")
    }
  }
...
.prepare_dashboard_context <- function() {
  ...
  final_aug <- final_data %>%
    dplyr::mutate(
      # FIX: Renamed from published_month to published_day for semantic correctness
      # (was flooring to day but calling it "month")
      published_day = lubridate::floor_date(published_date, "day"),
      published_month = lubridate::floor_date(published_date, "month"),  # FIX: Added actual monthly aggregation
      published_weekday = lubridate::wday(published_date, label = TRUE, abbr = TRUE)
    )

  authors_long <- final_aug %>%
    dplyr::select(paper_id, authors) %>%
  ...
  pubs_by_month <- final_aug %>%
    dplyr::count(published_month) %>%
    dplyr::arrange(published_month)

  pubs_by_day <- final_aug %>%
    dplyr::count(published_day) %>%
    dplyr::arrange(published_day)
  ...
  pubs_by_weekday <- final_aug %>%
    dplyr::filter(!is.na(published_weekday)) %>%
    dplyr::mutate(
    ...
    dplyr::count(published_weekday)

  author_counts <- final_aug %>%
    dplyr::filter(!is.na(authors)) %>%
    dplyr::mutate(n_authors = stringr::str_count(authors, ",") + 1)
...
.embedded_dashboard_template <- function() {
  c(
    ...
    "# FIX: Dynamic palette that handles any number of tags",
    "n_tags <- nrow(tag_counts)",
    "if (n_tags > 0) {",
    "  pal <- if (n_tags <= 12) RColorBrewer::brewer.pal(max(3, n_tags), \"Set3\") else colorRampPalette(RColorBrewer::brewer.pal(12, \"Set3\"))(n_tags)",
    "} else {",
    "  pal <- c(\"#999999\")",
    "}",
    "plot_ly(",
