...
#' Save publications to DuckDB (single table)
#'
#' Write a data.frame of papers into the package DuckDB database.
#' Uses an upsert pattern: UPDATE existing records, INSERT new ones.
#'
#' @param data data.frame as returned by `collect_all()` or processed data.
#'   Required columns: id, link, title, authors, abstract, categories,
#'   published_date, updated_date, tag, source, language.
...
save_publications <- function(data, db_path = NULL) {
  ...
  required <- c("id","link","title","authors","abstract","categories","published_date","updated_date","tag","source","language")
  ...
  df <- data.frame(
    ...
    language       = as.character(data$language),
    stringsAsFactors = FALSE
  )

  # FIX: Wrap upsert in a transaction for atomicity
  ...
  tryCatch({
    ...
    updated <- DBI::dbExecute(con, "
      ...
      SET
        ...
        source = COALESCE(NULLIF(trim(s.source), ''), p.source),
        language = COALESCE(NULLIF(trim(s.language), ''), p.language)
    ...
    inserted <- DBI::dbExecute(con, "
      INSERT INTO papers (
        ...
        categories, published_date, updated_date, tag, source, language
      )
      SELECT
        s.paper_id, s.link, s.title, s.authors, s.abstract,
        s.categories, s.published_date, s.updated_date, s.tag, s.source, s.language
      FROM stg_papers s
    ...
    stop("save_publications failed: ", conditionMessage(e))
  })
