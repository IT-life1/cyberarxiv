...
`cyberarxiv-mcp` поднимается на `http://localhost:5002/mcp` (streamable-http) и даёт AI-ассистенту 10 инструментов для поиска, классификации и загрузки статей. Простейший случай — Claude.ai web:

> Settings → Connectors → **Add custom connector** → URL `http://localhost:5002/mcp`.

Конфиги для Cursor, Claude Desktop (HTTP/stdio), mcp-inspector, описание всех 10 инструментов, развёрнутые примеры диалогов и заметки про безопасность — в отдельном README:

➡️ **[mcp_server/README.md](mcp_server/README.md)** — полная инструкция по MCP.
